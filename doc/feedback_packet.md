...
`host/lib/feedback/packet.py`（実機用デコーダの正本）でシミュレータ出力を復号すると
`checksum byte: 10` / `imu_yaw_deg: -89.85 deg` / `camera: (0,0,0,0)` /
`byte14: [0,1,2,3,4]` となることを framework 側が確認しています。
シミュレータ出力を `robot-feedback-viewer` などの実機ツールで見る用途も成立します。

### 残る差分: byte 52..59（速度）が指令途絶中に凍る
...
速度（byte 52/56）は破棄直前の値で凍ります**。framework セッションの実測では
ロボットが実際に停止したあとも 1.461 m/s を返し続けました。
...
実際にこれで壊れた測定があります。framework 側が停止距離を 1 プロセスで連続測定
したところ、**前の測定で凍った速度を「すでに目標速度が出ている」と読んで加速せずに
測定へ入り、全速の開始速度と停止距離 0 を報告しました**。対策は 1 測定 1 プロセスに
することです。**速度フィールドを判定に使うツールは、前の状態を引きずっていないかを
必ず確かめてください。**

配線し直さない判断は framework 側で、理由は `estimated_speed()` が実機 G474 の
オドメトリ相当であり、vision 微分に置き換えると**通常時のほうが実機から遠ざかる**
ためです。詳細と停止経路 2 種の対照は framework の
`docs/robot-side-position-control.md`「既知の忠実度ギャップ」を参照してください。
