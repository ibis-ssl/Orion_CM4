...
#### 【未決】基準側の停止タイムアウトをどこに合わせるか（2026-09-13）

素通し経路の stale 判定は `--command-timeout-ms`（**既定 100 ms**）を流用している。
実機の該当値は G474 の `connected_ai` タイムアウトで、framework セッションが
実機ソースで裏取りした通り **250 ms** である。

```c
// G474_Orion_main/Core/Src/ai_comm.c:8
#define AI_CMD_TIMEOUT (0.5)
// Core/Inc/management.h:74
#define MAIN_LOOP_CYCLE (500)
// ai_comm.c:169   500 * 0.5 = 250 ms
```

この乖離は A/B にとって**厄介な向き**に効く。

1. 基準側（旧構成）が実機より 2.5 倍速く止まる
2. **無線劣化を注入するほど「基準側だけが早く停止する」方向に働く**ので、
   「新構成の方が劣化に強い」という結論が、制御方式の優位ではなく
   タイムアウト値の差から出てしまう
...
**素通し専用のノブ（または CLI を持たない定数）を足す**形が良い。既定値を動かすと
新経路の挙動まで一緒に変わる。

判断は crane 側の A/B 基準側の設計判断と同じ人に集約されるべき事項である。
