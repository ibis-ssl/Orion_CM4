...
安全停止の停止距離は実機より 4.3 倍短く出ていました**（0.133 m → 修正後 0.571 m）。
実機より良く見える方向の誤差だったので、**PR #10 より前に取った停止距離の数字は
使えません**。

| 停止のかけ方（1.5 m/s から） | PR #10 前 | PR #10 後 |
|---|---|---|
| mode 3 で `r = 0` | 0.126 m | 0.126 m |
| `STOP_EMERGENCY` / vision 断 | 0.133 m | **0.571 m** |
| 指令が届かない（standby） | 0.573 m | 0.573 m |

修正後の 0.571 m が指令途絶時の 0.573 m と一致したことが、同じ経路に入った証拠です。
**0.571 m は比較対象であって実機の予測値ではありません**（duty 0 が空転か短絡制動かが
未確定なため）。
