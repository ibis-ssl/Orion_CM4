...
自己リセット待ち (約 6 秒) が要らなくなり、復帰は約 10 秒から「検知 + 起動 1.4 秒」になる。

- 実機確認 (2026-09-19、6 番): 正常な G474 にこの 1 フレームを送ると、40 ms 後に feedback が止まり、1.35 秒後に復帰した。
- 統合動作の実機確認 (同日、6 番): `robot_feedback.out` を 2.6 秒止めて無音を作ると、`無音 2001 ms` → `FWUP リセット指令を送信` →
  `feedback 復帰 (1450 ms 後)` (2 回とも 1450〜1459 ms)。PC 側でも復帰時に G474 リセット直後の値 (`BOOT_LIKE`) を確認した。
  緩和策だけの従来経路は 7942〜7966 ms 後 (実戦の 6・8 番の記録) なので、無音 1 回あたりのダウンは約 9.9 秒 → 約 3.5 秒。
- **未確認**: G474 の TX が本当に死んでいる最中に届いて効くか。上の試験は健全な G474 (受信は当然生きている) で行った。
  TX 死亡中も受信が生きていることは 7 番のデバッグ画面 (CM4 指令の受信を示す Dt が更新され続ける) で確認済みだが、
  本物の無音での実測は、自然発生 (実戦では約 30 秒おき) を待つ必要がある。
...
- **根本原因は G474 側の割り込み競合** (2026-09-19 特定。G474 側で直れば、この緩和は不要になる):
  ...
  - **「1 Mbps」「送受信を同時にすると壊れる」という見立てと一致する**:
    1 Mbps は 1 byte 10 us で、RX FIFO しきい値 1/8 のため USART2 割り込みが毎秒 4,500 回ほど走る (62.5 Hz x 72 byte)。
    危険窓は read-modify-write の約 3 命令 (170 MHz で約 18 ns) なので、危険時間は毎秒 8x10^-5。
    TX DMA 完了は 125 回/秒なので期待発生率は約 0.01 回/秒 = **MTBF 約 100 秒**。実測の 20〜150 秒と一致する。
    ボーレートや指令レートを下げれば比例して減る。
    一方これは**配線上の衝突ではない**: TX/RX はピンが別 (PB3/PB4) の全二重で、失敗時に PE/FE/NE/ORE は増えない。
    さらに TX と RX の重なり自体は毎分 900 回以上起きるのに故障は毎分 1 回程度で、約 1000 回に 1 回しか壊れない。
    これは命令レベルの狭い競合の特徴で、電気的な問題なら桁違いに高頻度で壊れるはずである。
  - **退行した時点**: `8560ff5 強制送信` (2026-03-21)。それ以前は
    「CM4 接続中は RX 完了時に送る / 未接続時だけタイマーで送る」(`if (connection.connected_cm4 == false)`) という
    **送受信を決して同時にしない設計**だった (`main.c:1327` のコメントはその名残)。この commit がその条件を外し、
    タイマー (500 Hz のうち 125 Hz) から常に送るようにしたため、CM4 が送信している間だけ重なるようになった。
  - **修正案** (G474 側): `stm32g4xx_it.c:364-365`(と `main.c:467-468`)の `SET_BIT` を **`ATOMIC_SET_BIT` に変える**。
    あるいは `8560ff5` 以前のように送受信を直列化し直す (ただし競合自体は残るので、FW ゲートウェイなど別経路で再発しうる)。
