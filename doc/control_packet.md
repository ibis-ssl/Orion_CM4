...
検査しています（200 Hz と 20 Hz で測って差が 30 ms 未満であること）。
`safety_changed` のバイパスを外すと 20 Hz 側が 148.7 ms になって落ちることを
確認済みです。**期待される「動かなさ」を検査にしておかないと、レートを上げ下げ
...
framework セッションがシミュレータで同じ対照を取った値が下表です。シミュレータの
`SimRobot::begin()` も最後の指令から 0.1 s で standby に入り、車輪 PID に到達する
手前で return するので、**この 2 経路の作り分けは実機とシミュレータで一致しています**。

ただし**この一致は 2026-09-13 に framework PR #10 で入ったもの**です。それ以前は
`ibisShouldStop()`（`stop_emergency` / vision 断 / vision タイムアウト）がゼロ速度の
`move_command` を出しており、車輪 PID を通る＝**能動制動**になっていました。上表の
とおり `applySafetyStop()` の安全停止はすべてこの経路なので、**シミュレータで測った
安全停止の停止距離は実機より約 5 倍短く出ていました**。実機より良く見える方向の
誤差だったので、**PR #10 より前に取った停止距離の数字は使えません**。

| 停止のかけ方（1.5 m/s から） | PR #10 前 | PR #10 後 |
|---|---|---|
| mode 3 で `r = 0`（能動制動） | 0.13 m | 0.13 m |
| `STOP_EMERGENCY` / vision 断 | 0.12〜0.14 m | **0.63 m** |
| 指令が届かない（standby） | 0.76 m | 0.76 m |

**0.63 m は比較対象であって実機の予測値ではありません**（duty 0 が空転か短絡制動かが
未確定なため）。

##### 数値は 2026-09-13 に一度訂正されています（framework PR #11）

当初 0.126 / 0.571 / 0.573 m として共有された値は、**ロボットが壁や他のロボットに
当たっていて自由な惰走を測れていませんでした**。塞がれた向きでは 0.12〜0.40 m の
範囲で不規則にばらつきますが、**値そのものは「それらしく」見えるので読みからは
異常と分かりません**。再測定の手順は framework の `data/scripts/ibis-stop-distance.py`
にあり、+y 方向で blue と yellow の両方を測って一致（0.02 m 以内）を PASS/FAIL で
判定する形になっています。

**「`STOP_EMERGENCY` の 0.571 m が指令途絶の 0.573 m と一致したので同じ経路の証拠」
という読みも誤りでした。** 両方が別々に遮られていた偶然です。正しくは**指令途絶の
方が約 0.13 m 長い**。`SimRobot` は最後の指令から 0.1 s 経つまで前の指令を実行し
続けるので、その間は駆動力がかかります。`STOP_EMERGENCY` は指令が届いた時点で
駆動を切るので短くなります。**コードから予測できた差**で、実測の一致を独立した
確認として扱ったために検算の機会を逃した、という経緯です。

##### 忠実度ギャップ: 指令途絶時はシミュレータの方が早く止まる

この standby までの **0.1 s は実機の 250 ms（`connected_ai` タイムアウト）より短い**
ので、**指令途絶時の惰走距離はシミュレータの方が実機より短く出ます**。
`doc/overview.md` の「基準側の停止タイムアウトをどこに合わせるか」（素通しの
stale 判定 100 ms vs 実機 250 ms）とは別経路ですが、**向きは同じ「シミュレータの方が
早く止まる」**です。安全側の評価をするときは両方効きます。
