...
## 制御指令のユニキャスト化（2026-09-27）

- Wi-Fiのブロードキャスト経路で約100ms単位の受信集中を確認したため、このrepoの制御指令の受信仕様を機体別65バイトのUDPユニキャストに切り替えた。設定パケットも各機体宛てに送る仕様とした。
- GUI_Qtの送信実装は`192.168.20.(100+ID):12345`へ`ID 1バイト + 指令64バイト`を送る。Orion_CM4の従来の715バイト入力は11個の65バイトスロットを連結したもので、スロット内のIDと64バイト指令の並びは一致していた。GUI_Qtはmode 3を送信し、craneの位置指令はmode 4と追加フィールドを使用する。
- `ai_cmd_v2.out`と`cm4_sim.out`の入力を65バイトに変更し、ID、長さ、空コマンドを検査する。シミュレータ向け715バイト出力はそのまま使用する。
- WSLでC++をビルドし、`cm4/build.sh`のレイアウト・位置制御・sim 18件・実機ブリッジ12件・タイミング1件のテストが成功。追加のmode3計測とUDP監視テスト3件、ホスト送信テスト6件も成功。実機への配備とcrane側送信器の切り替えは未実施。
